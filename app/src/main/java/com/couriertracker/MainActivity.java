...
public class MainActivity extends Activity{
 ...
 @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(0xff111318);getWindow().setNavigationBarColor(0xff0d0f13);
  ...
  web.setOnApplyWindowInsetsListener((v,in)->{int top,bottom;if(Build.VERSION.SDK_INT>=30){android.graphics.Insets x=in.getInsets(WindowInsets.Type.systemBars());top=x.top;bottom=x.bottom;}else{top=in.getSystemWindowInsetTop();bottom=in.getSystemWindowInsetBottom();}float d=getResources().getDisplayMetrics().density;String js="document.documentElement.style.setProperty('--top-inset','"+(top/d)+"px');document.documentElement.style.setProperty('--bottom-inset','"+(bottom/d)+"px');";v.setPadding(0,0,0,0);v.evaluateJavascript(js,null);return in;});web.loadUrl("file:///android_asset/index.html");
